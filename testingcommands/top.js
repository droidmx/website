module.exports.run = async (bot, msg, args) => {
    ...
    db.get(`Select * From "${db_settings}"`).then(settings => {
        db.all(`Select userrank, userid, balance From "${db_userdata}" Order By balance Desc`).then(server_data => {
            db.get(`Select * From "${db_userdata}" Where userid = ${msg.author.id}`).then(caller_row => {
                ...
                if (caller_row.userrank < 1) {
                    ...
                    if (server_members < 5){
                        var message = "The " + server_members + " richest on this server are: ";
                        var leaderboard = "\n\n";
                        ...
                        var message = "The 5 richest on this server are: ";
                        var leaderboard = "\n\n";
                        for (var i = 0; i < 5; i++)
                            leaderboard += "<@" + server_data[i].userid + ">:  $" + format(server_data[i].balance) + "\n";
                        msg.channel.send({
                    ...
                    try {
                        if (isNaN(parseInt(args[0]))){
                            ...
                            var message = "The " + top_num + " richest on this server are: ";
                            var leaderboard = "\n\n";
